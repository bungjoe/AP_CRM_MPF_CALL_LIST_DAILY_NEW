...
BEGIN 
    ...
    AP_PUBLIC.CORE_LOG_PKG.pStart('Ins:gtt_cmp_raw_cont_cc2');
		...
		and cc.skp_Client in (select nvl(skp_client,-9999) from camp_elig_base where eligible_final_flag = 1 and priority_actual > 0);
    AP_PUBLIC.CORE_LOG_PKG.pEnd ;
    commit;
  ...
  AND CC.SKP_CLIENT       IN
    (SELECT NVL(SKP_CLIENT,-9999)
    FROM CAMP_ELIG_BASE
    WHERE ELIGIBLE_FINAL_FLAG = 1
