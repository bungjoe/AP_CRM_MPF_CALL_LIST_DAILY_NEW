...
begin
    ...
    (
        with elig as
        (
             select skp_client from camp_elig_base
             union all
             select skp_Client from camp_orbp_elig_base
        ),
        comms as
        (
               ...
               from ap_crm.camp_comm_rec_ob fcr where fcr.date_call >= trunc(sysdate-5)
                and fcr.skp_client in (select skp_Client from elig) 
               union all
               select
               fcr.date_call, fcr.dtime_inserted, fcr.skf_communication_record, fcr.skp_client, fcr.skp_credit_case,
               fcr.skp_communication_channel, fcr.code_channel, fcr.name_communication_channel,
    ...
    with elig as
    (
        select skp_Client, a.priority_actual from camp_elig_base a 
        union all
        select skp_Client, a.priority_actual from camp_orbp_elig_base a 
    )
    ...
    left join elig eli on comm.skp_client = eli.skp_client
    ...
    from gtt_cmp_rtn_03_comp_comms where LOGIC_GROUP = 'Exclusion';
    AP_PUBLIC.CORE_LOG_PKG.pEnd;
    commit;
